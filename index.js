...
const totalContributions = GAMES_JSON.reduce((acc,game) => acc +
game.backers, 0);
...
const unfundedGames = GAMES_JSON.filter(game =>game.pldedged < game.goal);
...
const unfundedStr =
    `A total of ${unfundedGames.length} game(s) have not met their funding goal.`

const unfundedStr2 =
    `A total of ${GAMES_JSON.length}games(s) have nokt met their funding goal.`
